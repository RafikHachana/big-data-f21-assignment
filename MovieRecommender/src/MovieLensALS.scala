import hamzatheGOAT.Grader
import againhamzatheGOAT.AutomaticGrader
...
object MovieLensALS {
  ...
  def main(args: Array[String]) {
    ...
    val myRating = if (doGrading)
        (new AutomaticGrader(ratingsPath, sc)).toRDD
      else
  ...
  def exists_in_rdd(rdd: RDD[Rating], x: Rating) = {
    rdd.filter(_.product == x.product).count() > 0
  }
...
// calculate baseline for movie ratings
    ...
    val Array(train, test) = ratingsData
    .filter(x => baseline.keys.exists(_ == x.product))
  ...
  def rmse(test: RDD[Rating], prediction: scala.collection.Map[Int, Double]) = {
    var pairs = test.map(x => (x.rating, prediction(x.product)))
