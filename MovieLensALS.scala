...
//    }
    ...
    val schema = StructType("userID,movieID,movieRating,time".split(",").map(fieldName => {
      if(fieldName == "movieRating" )
        StructField(fieldName, DoubleType, nullable=false)
      else
        StructField(fieldName, IntegerType, nullable=false)
    }))

    val ratingsData = spark.read.format("csv").schema(schema).option("header", "true").load(path)
  ...
  def loadFilms(path: String, sc: SparkContext) = {
    ...
    val schema = StructType("movieId,title,genres".split(",").map(fieldName => {
    ...
    val data = spark.read.format("csv").schema(schema).option("header", "true").load(path)
